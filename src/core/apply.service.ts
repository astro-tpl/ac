...
export class ApplyService {
  ...
  private async applyDirectContent(
    ...
    if (options.dryRun) {
      ...
      logger.success(`内容已写入: ${targetPath}`)
    }
  }
  ...
  private async processTarget(
    ...
    if (mode === 'merge' && isJsonFile(absolutePath)) {
      try {
        ...
        logger.debug(`预览 JSON 合并失败: ${error.message}`)
      }
    }
  ...
  private async writeTargets(results: ApplyResult[], template: ContextTemplate): Promise<void> {
    ...
    for (const result of results) {
      try {
        ...
        logger.success(`已写入: ${result.targetPath}`)
      } catch (error: any) {
        logger.error(`写入失败 ${result.targetPath}: ${error.message}`)
      }
    }
    
    logger.success(`Context 模板应用完成: ${successCount}/${results.length} 个文件`)
  }
  ...
  private async readFromStdin(): Promise<string> {
    return new Promise((resolve, reject) => {
      ...
      process.stdin.on('error', (error) => {
        reject(new FileOperationError(`读取标准输入失败: ${error.message}`))
      })
    })
  }
  ...
  private createContentSummary(content: string, source: string): string {
    ...
    return `${lines} 行, ${chars} 字符 (来源: ${source})`
  }
  ...
  private validateOptions(options: ApplyOptions): void {
    ...
    if (sources === 0) {
      throw new FileOperationError(
        '必须指定内容来源: --context, --prompt, --content 或 --stdin'
      )
    }
    
    if (sources > 1) {
      throw new FileOperationError(
        '只能指定一个内容来源: --context, --prompt, --content 或 --stdin'
      )
    }
