...
import {t} from '../i18n'
import {
  IndexedTemplate, RepoConfig, Template, TemplateIndex,
} from '../types/index'
import {
  atomicWriteFile, fileExists, getFileStats, readFile, scanDirectory,
} from './fs'
...
export class IndexCacheManager {
  ...
  private async parseTemplateFile(filePath: string, repoName: string): Promise<IndexedTemplate | null> {
    try {
      ...
      logger.debug(t('index_cache.debug.parse_template_failed', {error: error.message, file: filePath}))
      return null
    }
  }
  ...
  private async scanRepoTemplates(repo: RepoConfig): Promise<IndexedTemplate[]> {
    ...
    try {
      ...
      logger.debug(t('index_cache.debug.scan_repo', {count: yamlFiles.length, name: repo.name}))
      ...
      logger.debug(t('index_cache.debug.repo_parse_success', {count: templates.length, name: repo.name}))
    } catch (error: any) {
      logger.warn(t('index_cache.warn.scan_repo_failed', {error: error.message, name: repo.name}))
    }
