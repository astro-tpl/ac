...
import {containsChinese, toPinyin} from './pinyin'
...
export class FuzzysortSearchEngine {
  ...
  highlightMatches(
    ...
    for (const fieldKey of matchedFields) {
      ...
      highlighted[fieldKey] = result ? fuzzysort.highlight(result, '<mark>', '</mark>') || fieldValue : fieldValue
    }
  ...
  private searchField(
    ...
    let bestScore = directResult ? directResult.score : Number.NEGATIVE_INFINITY
    ...
    // Apply weight
    return bestScore === Number.NEGATIVE_INFINITY ? Number.NEGATIVE_INFINITY : bestScore * weight
  }
  ...
  private searchTemplate(
    ...
    const {enablePinyin, threshold, weights} = options
    let totalScore = 0
